...
const growthDetail=document.querySelector('#growth-detail'),growthSteps=document.querySelectorAll('.growth-step');function setGrowthStep(i){growthSteps.forEach((s,idx)=>{s.classList.remove('active','passed');if(idx<i)s.classList.add('passed');if(idx===i)s.classList.add('active')});if(growthDetail)growthDetail.textContent=growthData[i]}growthSteps.forEach((step,i)=>step.addEventListener('click',()=>setGrowthStep(i)));if(growthSteps.length)setGrowthStep(0);
