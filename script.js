...
const growthFill=document.querySelector('#growth-fill'),growthDetail=document.querySelector('#growth-detail'),growthSteps=document.querySelectorAll('.growth-step');function setGrowthStep(i){growthSteps.forEach(s=>s.classList.remove('active'));const step=growthSteps[i];if(!step)return;step.classList.add('active');if(growthFill)growthFill.style.width=`${i/(growthData.length-1)*100}%`;if(growthDetail)growthDetail.textContent=growthData[i]}growthSteps.forEach((step,i)=>step.addEventListener('click',()=>setGrowthStep(i)));if(growthSteps.length)setGrowthStep(0);
